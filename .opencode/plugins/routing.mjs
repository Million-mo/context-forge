...
function showGuidanceOnce(type, sessionId) {
...
function buildGuidanceContext(type, sessionId) {
...
const DANGEROUS_ROUTING_PATTERNS = [
    /curl.*\|.*sh/i,
    /wget.*\|.*sh/i,
    /\$\([^)]*\).*\|.*sh/i,
    /`[^`]*`.*\|.*sh/i,
...
function hasDangerousPattern(cmd) {
...
function isStructurallyBounded(cmd) {
    if (!cmd || SHELL_CONTROL_OPERATORS.test(cmd.trim()))
        return false;
    return SAFE_COMMAND_PATTERNS.some(rx => rx.test(cmd.trim()));
}
function isUnboundedOutput(cmd) {
...
function routeBash(command, sessionId, mcpReady) {
    if (!command)
        return { action: "allow" };
...
function routeTool(tool, args, sessionId, mcpReady) {
