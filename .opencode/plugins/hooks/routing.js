...
function isStructurallyBounded(command) {
    if (!command)
        return false;
...
function isBuildTool(command) {
    return BUILD_TOOL_PATTERNS.some(rx => rx.test(command));
}
function isUnboundedOutput(command) {
...
// ─────────────────────────────────────────────────────────
function routeBash(command, args, sessionId, mcpReady) {
    if (!command)
        return { action: "allow" };
...
function routeRead(args, sessionId, mcpReady) {
...
function routeGrep(args, sessionId, mcpReady) {
    if (!mcpReady)
        return { action: "allow" };
...
function routeWebFetch(sessionId, mcpReady) {
    if (!mcpReady) {
        return { action: "allow" };
    }
...
function routeEdit(args, sessionId) {
    // Edit operations are generally safe
    return { action: "allow" };
}
function routeWrite(args, sessionId) {
...
function routeAgent(args, sessionId) {
    // Agent/subagent: inject routing block into prompt if available
    // For now, allow
    return { action: "allow" };
}
function routeExternalMcp(sessionId) {
