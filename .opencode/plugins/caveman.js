 ...
 *
 * Source of truth: ctx_plugin/src/caveman.ts
...
function safeWriteFlag(flagPath, content) {
    try {
        ...
        try {
            if (lstatSync(realFlagPath).isSymbolicLink())
                return;
        }
        catch (e) {
            if (e.code !== "ENOENT")
...
function applyDefaultMode() {
    ...
    if (mode === "off") {
        try {
            if (existsSync(CAVEMAN_FLAG))
                unlinkSync(CAVEMAN_FLAG);
        }
        catch { }
    }
    else {
...
function parseModeChange(prompt) {
    ...
    if (!p)
        return null;
    if (/\b(stop|disable|deactivate|turn off)\b.*\bcaveman\b/i.test(p) ||
        /\bcaveman\b.*\b(stop|disable|deactivate|turn off)\b/i.test(p) ||
        /\bnormal mode\b/i.test(p))
        return "off";
    if (/\b(activate|enable|turn on|start|talk like)\b.*\bcaveman\b/i.test(p) ||
