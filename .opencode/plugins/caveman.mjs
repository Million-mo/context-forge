 ...
 *
 * Inlined from ctx_plugin/src/plugin.ts — opencode auto-loads this from
 * .opencode/plugins/. Edit ctx_plugin/src/plugin.ts and copy here, or run
 * bin/install.js to regenerate.
 *
 * RTK: intercepts bash/shell tool calls, rewrites via `rtk rewrite`.
 * Caveman: tracks compression mode, parses /caveman + natural language,
 * loads SKILL.md, and emits per-turn reinforcement lines.
 *
 * Hooks used:
 *   - session.created  → Initialize caveman flag on startup
 *   - tool.execute.before → RTK command rewriting
 *   - chat.message     → Mode tracking + reinforcement
 */
...
export const CtxPlugin = async (input) => {
  ...
  return {
    ...
    "chat.message": async (input, output) => {
      ...
      // Reinforcement — append if caveman is active
