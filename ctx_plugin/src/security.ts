...
export interface SecurityResult {
  action: "allow" | "deny" | "ask" | "modify" | "context";
  reason?: string;
...
function getOpencodeConfigDir(): string {
  ...
  if (process.env.XDG_CONFIG_HOME) return resolve(process.env.XDG_CONFIG_HOME, "opencode");
...
export function readBashPolicies(
  ...
  if (global) policies.push(global);
...
): { decision: PermissionDecision; matchedPattern?: string } {
...
export function checkSecurityPolicy(args: {
  ...
  if (tool === "bash" || tool === "shell") {
    if (!args.command) return { action: "allow" };
  ...
  if (tool === "read") {
    if (!args.path) return { action: "allow" };
